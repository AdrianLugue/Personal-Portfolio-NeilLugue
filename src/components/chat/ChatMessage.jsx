...
function renderMarkdown(text) {
  return text.split('\n').map((line, i) => {
    ...
    const parts = line.split(/\*\*(.*?)\*\*/g).map((part, j) =>
      j % 2 === 1 ? <strong key={j} className="text-[#FFD54F] font-semibold">{part}</strong> : part
    );
    // Bullet lines starting with •
    const isBullet = line.trimStart().startsWith('•');
    return (
      <span key={i} className={`block ${isBullet ? 'pl-2 text-neutral-200' : ''} ${i > 0 ? 'mt-1.5' : ''}`}>
        {parts}
      </span>
    );
  });
...
function ActionButton({ action, onClose }) {
  ...
  return (
    <button
      onClick={handleClick}
      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#18140E] hover:bg-[#251E12] border border-[#C6B99B]/30 hover:border-[#FFD54F]/60 text-neutral-200 hover:text-[#FFD54F] font-montserrat font-semibold text-[10.5px] tracking-wide uppercase transition-all duration-200 cursor-pointer shadow-sm hover:shadow-[0_0_12px_rgba(255,213,79,0.2)]"
    >
...
export default function ChatMessage({ message, onClose, onChipClick, usedChips }) {
  ...
  if (!isBot) {
    ...
    return (
      <div className="flex justify-end">
        <div className="max-w-[82%] px-3.5 py-2.5 rounded-2xl rounded-br-sm bg-gradient-to-br from-[#2F2618] to-[#1F180E] border border-[#FFD54F]/35 shadow-[0_4px_14px_rgba(0,0,0,0.5)] text-white font-montserrat text-xs leading-relaxed">
          {message.text}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex gap-2.5 items-start">
      ...
      <div className="shrink-0 w-7 h-7 rounded-full bg-gradient-to-br from-[#2A2418] to-[#14100B] border border-[#FFD54F]/40 flex items-center justify-center text-[#FFD54F] select-none mt-0.5 shadow-sm">
        <Bot size={14} />
      </div>

      <div className="flex-1 min-w-0">
        {/* Bubble */}
        <div className="px-3.5 py-3 rounded-2xl rounded-tl-sm bg-[#14120E] border border-[#C6B99B]/20 text-neutral-200 font-montserrat text-xs leading-[1.65] break-words shadow-[0_4px_16px_rgba(0,0,0,0.6)]">
          {isLoading ? (
            <span className="flex items-center gap-1.5 text-neutral-400 font-mono text-[11px]">
              <span className="inline-flex gap-1">
                {[0, 1, 2].map((i) => (
                  <span
                    key={i}
                    className="w-1.5 h-1.5 rounded-full bg-[#FFD54F] animate-bounce"
            ...
            <>
              ...
              {!isFinished && (
                <span className="inline-block w-0.5 h-3 bg-[#FFD54F] ml-0.5 align-middle animate-pulse" />
              )}
